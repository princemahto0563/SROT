...
export function Panel({
  ...
  return (
    <section className={`panel shadow-panel ${className}`}>
      {(title || right) && (
        <header className={`flex items-start justify-between gap-4 ${pad ? "px-5 pt-4" : "px-4 pt-3"}`}>
          <div>
            {title && <h2 className="lbl">{title}</h2>}
            {hint && <p className="mt-1 text-[11.5px] leading-relaxed text-muted max-w-[70ch]">{hint}</p>}
          </div>
          {right}
        </header>
      )}
      <div className={pad ? "p-5 pt-4" : "p-4 pt-3"}>{children}</div>
    </section>
  );
}
...
/* ── Stat tile ────────────────────────────────────────────────────────────── */
export function Stat({ label, value, sub, tone = "ink" }: { label: string; value: string; sub?: string; tone?: "ink" | "danger" | "amber" }) {
  const c = tone === "danger" ? "text-danger" : tone === "amber" ? "text-amber" : "text-ink";
  return (
    <div className="panel shadow-panel px-5 py-4">
      <div className="lbl">{label}</div>
      <div className={`mt-2 text-[30px] font-bold leading-none tabular-nums ${c}`}>{value}</div>
...
export function Notice({
  ...
  const cfg = {
    info: { c: "text-ink2 border-line bg-s2", I: Info },
    warn: { c: "text-amber border-amber/35 bg-amber/[0.07]", I: AlertTriangle },
    safe: { c: "text-ok border-ok/35 bg-ok/[0.07]", I: ShieldCheck },
  }[kind];
  const I = cfg.I;
  return (
...
export function PageHead({
  ...
  return (
    <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
      <div>
        <div className="lbl text-accent">{eyebrow}</div>
        <h1 className="mt-1.5 text-[23px] font-bold tracking-[-0.01em] text-ink">{title}</h1>
        {sub && <p className="mt-1.5 max-w-[86ch] text-[12.5px] leading-relaxed text-ink2">{sub}</p>}
      </div>
      {right}
    </div>
  );
...
export function Mono({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <span className={`font-mono text-[11.5px] tracking-tight ${className}`}>{children}</span>;
}
  ...
  return (
    <div className="space-y-2.5" role="status" aria-busy="true">
      ...
      {Array.from({ length: rows }).map((_, i) => (
        <Skeleton key={i} h={i === 0 ? 20 : 13} w={i === 0 ? "42%" : `${96 - i * 7}%`} />
      ))}
    </div>
  );
}
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-start gap-2.5 rounded-lg border border-dashed border-line bg-s2/50 px-5 py-6">
      <div className="text-[13px] font-semibold text-ink2">{title}</div>
...
export function ErrorState({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div className="rounded-lg border border-danger/35 bg-danger/[0.07] px-4 py-3.5">
      <div className="flex items-start gap-2.5">
        <AlertTriangle size={14} className="mt-[2px] shrink-0 text-danger" />
        <div className="min-w-0">
          <div className="text-[12.5px] font-semibold text-danger">Could not load this view</div>
          <p className="mt-1 break-words text-[11.5px] leading-relaxed text-ink2">{message}</p>
          {onRetry && (
            <button onClick={onRetry} className="btn btn-ghost mt-2.5 text-[11px]">Try again</button>
          )}
        </div>
      </div>
    </div>
      ...
      <table className="w-full border-collapse text-[11.5px]">
        <thead>
          <tr className="border-b border-line">
            {head.map((h, i) => (
              <th key={i} className="whitespace-nowrap px-2.5 py-2 text-left text-[9.5px] font-semibold uppercase tracking-[0.09em] text-muted">
                {h}
              </th>
            ))}
          </tr>
...
export function Row({ children, tone = "" }: { children: ReactNode; tone?: string }) {
  return <tr className={`border-b border-lineSoft last:border-0 ${tone}`}>{children}</tr>;
}

export function Cell({ children, className = "", mono = false }: { children?: ReactNode; className?: string; mono?: boolean }) {
  return (
    <td className={`px-2.5 py-2 align-top text-ink2 ${mono ? "font-mono text-[11px]" : ""} ${className}`}>
      {children ?? "—"}
    </td>
  );
}
...
export function Field({ label, children, mono = false }: { label: string; children?: ReactNode; mono?: boolean }) {
  return (
    <div className="min-w-0">
      <div className="lbl">{label}</div>
      <div className={`mt-1 break-words text-[12px] text-ink ${mono ? "font-mono text-[11px]" : ""}`}>
        {children ?? <span className="text-muted">Not available</span>}
      </div>
    </div>
  );
}
